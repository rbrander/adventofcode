...
function addDistance(distance, onChangeIncrement) {
  ...
  for (var i = 0; i < distance; i++) {
    x += xDirection;
    y += yDirection;
    onChangeIncrement({ x, y });
  }
}

let foundFirstOverlap = false;
function onChange(newPosition) {
  if (foundFirstOverlap) return;
  if (placesVisited.some(d => (d.x == newPosition.x && d.y == newPosition.y))) {
    console.log('x =', newPosition.x, 'y =', newPosition.y, 
      ' dist =', Math.abs(newPosition.x) + Math.abs(newPosition.y));
    foundFirstOverlap = true;
  }
  placesVisited.push(newPosition);
}
...
commands.forEach(command => {
  ...
  changeDirection(dir);
  addDistance(dist, onChange);
})

// answer: x = -11 y = -131  dist = 142
