...
// store visited positions, where key = tuple (x,y); value = 1+
const visited = claims.reduce((visited, curr) => {
  ...
  for (var x = curr.x; x < curr.x2; x++) {
    for (var y = curr.y; y < curr.y2; y++) {
      const key = `(${x},${y})`;
      visited[key] = key in visited ? visited[key] + 1 : 1;
    }
  }
  return visited;
}, {});

// now count all the keys where value is > 1
const totalSquareInches = Object.entries(visited)
  .reduce((count, [_, currCount]) => currCount > 1 ? count + 1 : count, 0);

console.log('Total Square Inches overlapping:', totalSquareInches); // 124850
