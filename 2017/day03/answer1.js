...
const getNumSteps = (numToFind) => {
  ...
  let x = 0;
  let y = 0;
  console.log('i  ringLevel  sideOffset  totalSteps');
  console.log('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~');
  // memory.push({ value: 1, x: 0, y: 0 });
  // console.log(` ${1}) = (${x}, ${y})`);
  for (var i = 1; i <= 10; i++) {
    const sqrt = Math.sqrt(i);
    const ringLevel = Math.ceil((Math.ceil(sqrt) + 1)/2);
    const size = (ringLevel * 2);
    // const offset = i - (((size - 2) ** 2) + 1); // from last ring level
    const offset = i - ((ringLevel - 1) ** 2);
    const side = ~~(offset / size); // number between 0 and 3
    const sideOffset = (offset % size) || 0;
    const totalSteps = ringLevel + sideOffset;
...
console.log(' numSteps 3 = ', getNumSteps(3));
