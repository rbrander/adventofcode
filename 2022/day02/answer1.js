...
const isRock = (value) => ['A', 'X'].includes(value);
const isPaper = (value) => ['B', 'Y'].includes(value);
const isScissors = (value) => ['C', 'Z'].includes(value);
...
const getOutcomeScore = (firstValue, secondValue) => {
  if (valueScore[firstValue] === valueScore[secondValue]) {
    return OUTCOME_SCORES[OUTCOME_DRAW];
  }
  const isFirstValueBetter = getIsFirstBetter(firstValue, secondValue);
...
const calcScore = (firstValue, secondValue) => {
  ...
  return valueScore[secondValue] + outcomeScore;
};

let totalPoints = values.map(valuePair => calcScore(...valuePair)).reduce((sum, curr) => sum + curr);

console.log('Total points won:', totalPoints); // 12586
