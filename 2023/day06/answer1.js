...
*/
console.log();
let numWinningDistances = []
for (let round = 0; round < times.length; round++) {
  const totalTime = times[round];
  const distancesGreaterThanMax = []
  for (let holdingTime = 1; holdingTime < totalTime; holdingTime++) {
    ...
    if (distance > distances[round]) {
      distancesGreaterThanMax.push(distance);
    }
  }
  numWinningDistances.push(distancesGreaterThanMax.length);
}
const answer = numWinningDistances.reduce((answer, curr) => answer * curr);
console.log('answer:', numWinningDistances.join(' * '), '=', answer);
