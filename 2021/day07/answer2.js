...
let cache = [0, 1];
// Will return the sum of the fuel cost for a distance provided
function memoizeSum(distance) {
  const hasCache = distance in cache
  if (hasCache) return cache[distance]

  // Used iterative computation instead of recursive (due to call stack size)
  let sum = 0
  for (let i = 1; i <= distance; i++) {
    sum += i
    const shouldSetCache = i in cache === false
    if (shouldSetCache) {
      cache[i] = sum
    }
  }
...
const calculateFuelCost = (positions, destinationPosition) =>
  positions.reduce((fuelCost, position) => fuelCost + memoizeSum(Math.abs(destinationPosition - position)), 0)
...
// The best alignment is 489 costing 99788435 fuel
