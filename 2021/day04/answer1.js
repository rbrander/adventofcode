...
drawNumbers.some(number => // draw a number
  boards.some((board, idx) => {
    // mark boards with number
    markBoard(board, idx, number)
    // check for win
    hasWon = checkForWin(board, idx)
    lastNum = number
    if (hasWon) {
      boardIndexWon = idx
    }
...
const sum = sumUnmarkedCells(boardIndexWon)
console.log({hasWon, boardIndexWon, lastNum, sum })
console.log(`lastNum x sum = ${lastNum} x ${sum} = ${lastNum * sum}`)
// lastNum x sum = 94 x 621 = 58374
